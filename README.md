# Rodney Dhavid Jimenez Chacin

> _"...And you'll do it despite rejection and the worst odds and it will be better than anything else you can imagine."_ — Charles Bukowski, Roll the Dice

---

<div align="center">

**Cybersecurity | Full-Stack Developer | Builder of Intelligent Systems**

Crafting security tools, intelligent agents, and scalable systems with precision and passion.

[🌐 Portfolio](https://rodhnin.com) • [📧 Email](mailto:dhavidjimenez@outlook.com) • [𝕏 X](https://x.com/rodhnin)

</div>

---

## 🔐 About

Builder passionate about security, artificial intelligence, and elegant code. I specialize in developing sophisticated systems that combine **vulnerability detection**, **real-time AI integration**, and **high-performance backend architecture**.

My work spans:

-   **Offensive Security**: Vulnerability scanning, network auditing, evidence collection
-   **Defensive Engineering**: Hardening guides, ethical automation, consent-based testing
-   **Full-Stack AI Systems**: LangChain agents, streaming APIs, interactive frontends
-   **Ethical Security Tools**: Production-ready scanners with ethical design patterns

Every tool I build prioritizes **authorization**, **transparency**, and **professional-grade results**.

---

## 🛠️ Core Stack

### Languages

![Python](https://img.shields.io/badge/Python-3.11+-3776ab?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-5.0+-3178c6?style=flat-square&logo=typescript&logoColor=white)
![C#](https://img.shields.io/badge/C%23-11.0+-239120?style=flat-square&logo=c-sharp&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-8.0+-777bb4?style=flat-square&logo=php&logoColor=white)
![C](https://img.shields.io/badge/C-ANSI-A8B9CC?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-17+-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-5.1+-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-7+-5391FE?style=flat-square&logo=powershell&logoColor=white)

### Backend

![FastAPI](https://img.shields.io/badge/FastAPI-0.104+-009688?style=flat-square&logo=fastapi&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-18+-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-8.0+-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1.0+-000000?style=flat-square)

### Frontend

![Next.js](https://img.shields.io/badge/Next.js-14+-000000?style=flat-square&logo=nextjs&logoColor=white)
![React](https://img.shields.io/badge/React-18+-61dafb?style=flat-square&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-Latest-3178c6?style=flat-square&logo=typescript&logoColor=white)
![Three.js](https://img.shields.io/badge/Three.js-r170+-000000?style=flat-square&logo=three.js&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-3.3+-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)

### Databases

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-15+-336791?style=flat-square&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-7+-DC382D?style=flat-square&logo=redis&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-Latest-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-3+-003B57?style=flat-square&logo=sqlite&logoColor=white)

### AI & ML

![OpenAI](https://img.shields.io/badge/OpenAI-GPT--4-412991?style=flat-square&logo=openai&logoColor=white)
![Claude API](https://img.shields.io/badge/Claude-3--Sonnet-000000?style=flat-square)
![LangChain](https://img.shields.io/badge/LangChain-Agents-000000?style=flat-square)
![Ollama](https://img.shields.io/badge/Ollama-Local%20LLM-000000?style=flat-square)

### DevOps & Infrastructure

![Docker](https://img.shields.io/badge/Docker-Latest-2496ED?style=flat-square&logo=docker&logoColor=white)
![Docker Compose](https://img.shields.io/badge/Docker%20Compose-Latest-2496ED?style=flat-square&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-CI/CD-2088FF?style=flat-square&logo=github-actions&logoColor=white)
![Railway](https://img.shields.io/badge/Railway-Deployment-0B0D0E?style=flat-square&logo=railway&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-Hosting-000000?style=flat-square&logo=vercel&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-Cloud-FF9900?style=flat-square&logo=amazon-aws&logoColor=white)

### Security Tools

![Metasploit](https://img.shields.io/badge/Metasploit-Framework-003366?style=flat-square)
![Burp Suite](https://img.shields.io/badge/Burp%20Suite-Professional-FF6600?style=flat-square)
![Wireshark](https://img.shields.io/badge/Wireshark-Network-1679A7?style=flat-square&logo=wireshark&logoColor=white)
![Snort](https://img.shields.io/badge/Snort-IDS-FF0000?style=flat-square)
![YARA](https://img.shields.io/badge/YARA-Signatures-000000?style=flat-square)
![OWASP](https://img.shields.io/badge/OWASP-ASVS-000000?style=flat-square)
![Mitre ATT&CK](https://img.shields.io/badge/Mitre-ATT%26CK-FF0000?style=flat-square)

### System Administration

![Linux](https://img.shields.io/badge/Linux-Advanced-FCC624?style=flat-square&logo=linux&logoColor=black)
![NGINX](https://img.shields.io/badge/NGINX-Reverse%20Proxy-009639?style=flat-square&logo=nginx&logoColor=white)
![Apache](https://img.shields.io/badge/Apache-Web%20Server-D09B26?style=flat-square&logo=apache&logoColor=white)
![Git](https://img.shields.io/badge/Git-Version%20Control-F05032?style=flat-square&logo=git&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-API%20Testing-FF6C37?style=flat-square&logo=postman&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-Automation-221F1F?style=flat-square&logo=n8n&logoColor=white)

---

## 🎯 Featured Work

My projects demonstrate expertise in **security automation**, **AI-powered analysis**, and **production-grade systems**:

### 🔮 Pythia — SQL Injection Detection Scanner

Advanced vulnerability detection with 4 specialized methods and AI-powered remediation guides.

-   **4 Detection Methods**: Error-based, Boolean-blind, Time-based, UNION-based
-   **AI-Powered Hardening Guides** (technical + executive modes)
-   **Status**: _Version 0.1.0_

[👉 Explore Pythia on GitHub](https://github.com/rodhnin/pythia-sql-clairvoyance)

### ⚒️ Hephaestus — Server Security Auditor

Comprehensive configuration auditor for Apache, Nginx, and IIS with intelligent hardening guidance.

-   **6 Security Modules**: Headers, TLS/SSL, HTTP Methods, Directory Listing, Sensitive Files, Version Disclosure
-   **AI-Powered Hardening Guides** (technical + executive modes)
-   **Status**: _Version 0.1.0_

[👉 Discover Hephaestus on GitHub](https://github.com/rodhnin/hephaestus-server-forger)

### 👁️ Argus — WordPress Security Scanner

Ethical WordPress vulnerability scanner with consent-based testing and AI analysis.

-   **WordPress-Specific Checks**: Plugins, themes, sensitive files, user enumeration
-   **AI-Powered Hardening Guides** (technical + executive modes)
-   **Status**: _Version 0.1.0_

[👉 Check out Argus on GitHub](https://github.com/rodhnin/argus-wp-watcher)

### 🌐 Asterion — Network Security Auditor

Multi-threaded network and domain security auditor with Active Directory analysis.

-   **SMB, RDP, LDAP, Kerberos, DNS** security assessments
-   **Windows & Linux** system hardening checks
-   **AI-Powered Hardening Guides** (technical + executive modes)
-   **Status**: _Version 0.1.0_

[👉 Explore Asterion on GitHub](https://github.com/rodhnin/asterion-network-minotaur)

### 🤖 Poiesis AI — Interactive Portfolio System

Full-stack AI-powered portfolio with intelligent consulting chatbot and lead generation.

-   **LangChain Agent** with 6 specialized tools
-   **Session Management**: Redis + PostgreSQL
-   **Frontend**: Next.js | **Backend**: FastAPI
-   **Status**: _Version 0.1.0_

[👉 Try the AI Agent Live](https://rodhnin.com)

### 📚 Acadelia — AI-Powered Learning Platform

Ambitious educational platform with RAG, multi-agent systems, and multimodal processing.

-   **40+ Specialized Agents** by subject area
-   **100% Supabase RAG**: pgvector + hybrid search
-   **4 Chat Styles**: PDF, Audio/Video, Mathematical, Theoretical
-   **8-Layer Security**: Helmet, CORS, CSRF, JWT, Rate Limiting, ClamAV, RBAC, Monitoring
-   **Multimodal**: PDF processing, Whisper (audio), GPT-4o Vision
-   **Status**: MVP - Portfolio Showcase

[👉 View Acadelia on GitHub](https://github.com/rodhnin/acadelia)

---

## 📊 What Drives Me

-   **Precision in Security**: Ethical testing, consent verification, zero false positives
-   **Intelligent Automation**: AI agents that reason and adapt
-   **Beautiful Architecture**: Clean code, scalable systems, professional-grade quality
-   **Continuous Learning**: Cybersecurity, machine learning, system design
-   **Philosophy & Creation**: Beyond code — building tools that matter

**Interests**: Cybersecurity • Machine Learning • Robotics • Web Development • Philosophy • Mythology • Brazilian Jiu-Jitsu

---

## 🔗 Connect

-   **Website**: [rodhnin.com](https://rodhnin.com) — Try the AI agent live
-   **GitHub**: [@rodhnin](https://github.com/rodhnin)
-   **X**: [@rodhnin](https://x.com/rodhnin)

Open to collaborations on security research, full-stack AI projects, and technical challenges.

---

<div align="center">

**Built with precision, passion, and persistence.**

_Rolling the dice despite the odds._

</div>
